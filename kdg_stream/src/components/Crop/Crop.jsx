...
export default function Crop ({Image, currentImage , setIsShowCrop , setImage ,ImagePos , setImagePos}) {
    const [crop, setCrop] = useState({x : 0 , y : 0})
    const [zoom, setZoom] = useState(1000)
    const onCropComplete = useCallback(({x,y, width}, croppedAreaPixels) => {
        console.log(x,y);
    ...
    const handleUploadAvatar = useCallback(async () => {
        ...
        const res_pos = await callAPI.post('/avatar_pos', ImagePos)
        if (res.status === 1) {
        }
    },[ImagePos])
    return (
