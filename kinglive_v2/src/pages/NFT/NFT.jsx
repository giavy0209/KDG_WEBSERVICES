...
export default function NFT() {
  ...
  const getAssets = useCallback(async () => {
    ...
    const res = await callAPI.get(
      `/listing-asset?limit=9&${ids.length ? `ids=${ids}` : ''}`,
      true,
        ...
        if (token.address === top9List[ActiveTop9].payment_token) {
          setTotal(
            new Decimal(amount).mul(
              Decimal(top9List[ActiveTop9].price).div(new Decimal(10).pow(token.decimal))
            )
          )
        }
      })
          ...
          <form className='containerX' onSubmit={handleBuy} onClick={(e) => e.stopPropagation()}>
            <div className='form-control'>
              ...
              <input
                ...
                name='_name'
                readOnly
                value={top9List[ActiveTop9]?.asset?.metadata?.name}
              />
              <input
                type='hidden'
                name='_contract'
                readOnly
                value={top9List[ActiveTop9]?.asset?.collection_id}
              />
            ...
            <div className='form-control'>
              ...
              <input
                type='number'
                readOnly
                value={top9List[ActiveTop9]?.quantity}
                name='_quantity'
              />
            </div>
            <div className='form-control'>
              ...
              <input
                ...
                name='_amount'
                min='1'
                max={top9List[ActiveTop9]?.quantity}
