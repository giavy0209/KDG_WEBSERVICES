...
const MyArtworkDetail = () => {
  ...
  const isOwner = useMemo(
    () => userRedux?.address === userAssetList[currentIndex]?.user?.address,
    [userRedux?.address, userAssetList, currentIndex]
  )
  ...
  useEffect(() => {
    ;(async () => {
      if (!account) return
      contractKL1155?.isApprovedForAll(account, addressMarket).then((approved) => {
  ...
  return (
    <>
      {isOpenSell && (
        <div
          key={userAssetList[currentIndex]?._id}
          className='popupX'
            ...
            <div className='contents_box'>
              <div className='form-control'>
                <img
                  ...
                  src={userAssetList[currentIndex]?.asset?.metadata?.image}
                  alt=''
                />
                ...
                <div className='items_information'>
                  ...
                  <p>
                    {userAssetList[currentIndex]?.asset?.metadata?.mimetype}
                    <br />
                  </p>
                  {/* ---e:Type--- */}
                </div>
                {/* --e:form-items_information-- */}
                ...
                <div className='flex_column'>
                  ...
                  <div className='input_boundingbox'>
                    ...
                    <input
                      ...
                      id='_id'
                      name='_id'
                      value={userAssetList[currentIndex]?.asset?.id}
                    />
                    ...
                    <input
                      ...
                      id='_quantity'
                      min='1'
                      max={userAssetList[currentIndex].amount}
              ...
              <div className='form-control submit_box'>
                ...
                <button className='buttonX--cancel' onClick={() => setIsOpenSell(false)}>
                  Cancel{' '}
                </button>
              </div>
              {/* --------------e:form-control------------------------ */}
      ...
      <div className='main-wrapper'>
        <div className='box-slide-artwork'>
          ...
          <div
            className='box-img'
            onClick={() =>
              window.open(userAssetList[currentIndex]?.asset?.metadata?.image, '_blank')
            }
          >
            <img src={zoom} alt='' />
          </div>
        ...
        <div className='box-artist-wrapper'>
          <div className='artist-content'>
            <div className='artist-content-top'>
              <p className='text-artist'>Artists</p>
              <h2 className='title-artist'>{userAssetList[currentIndex]?.asset?.metadata?.name}</h2>
              <p className='desc-artist'>
                ...
                <span className='txt'>
                  {userAssetList[currentIndex]?.amount} of{' '}
                  {userAssetList[currentIndex]?.asset.total_editions}
                </span>
              </p>
            </div>
              ...
              <div className='desciption-artist'>
                <p className='desc'>
                  ...
                  <span className='color-fff'>
                    {' '}
                    {userAssetList[currentIndex]?.user?.kyc?.last_name
                      ? userAssetList[currentIndex]?.user?.kyc?.last_name +
                        ' ' +
                        userAssetList[currentIndex]?.user?.kyc?.first_name
                      : userAssetList[currentIndex]?.user?.address}{' '}
                  </span>
                </p>
                {/* <p className="desc">
                ...
                <p className='desc'>
                  ...
                  <span className='color-fff'>
                    {' '}
                    {new Date(userAssetList[currentIndex]?.asset?.time).toDateString()}
                  </span>
                </p>
              ...
              <div className='artist-contract'>
                <div className='contract-item'>
                  ...
                  <p className='address-contract'>
                    {userAssetList[currentIndex]?.asset?.collection_id?.substring(0, 5) +
                      '...' +
                      userAssetList[currentIndex]?.asset?.collection_id?.substring(
                        userAssetList[currentIndex]?.asset?.collection_id.length - 9,
                        userAssetList[currentIndex]?.asset?.collection_id.length
                      )}{' '}
                    <img src={address} alt='' />
                  </p>
                </div>
                ...
                <div className='contract-item'>
                  ...
                  <p className='address-contract'>
                    {userAssetList[currentIndex]?.user?.address.substring(0, 5) +
                      '...' +
                      userAssetList[currentIndex]?.user?.address.substring(
                        userAssetList[currentIndex]?.user?.address.length - 9,
                        userAssetList[currentIndex]?.user?.address.length
                      )}{' '}
                    <img src={address} alt='' />
                  </p>
                </div>
            ...
            {isReviewer && !isOwner && userAssetList[currentIndex]?.asset?.status === 2 && (
              <div className='artist-content-status-reject'>Rejected</div>
            )}
            {isReviewer && !isOwner && userAssetList[currentIndex]?.asset?.status === 1 && (
              <div className='artist-content-status-accept'>Accepted</div>
            )}
          </div>
        </div>
      </div>
