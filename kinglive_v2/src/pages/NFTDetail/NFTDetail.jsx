...
import SwiperCore, { EffectFlip, Lazy, Navigation } from 'swiper/core'
...
const NFTDetail = () => {
  ...
  const isOwner = useMemo(
    () => userRedux?.address === marketList[currentIndex]?.owner?.address,
  ...
  const isApproval = useMemo(async () => {
    ...
    if (allowance && marketList[currentIndex]?.price) {
      if (
        new Decimal(allowance).gt(
          new Decimal(marketList[currentIndex].price).mul(marketList[currentIndex]?.quantity)
        )
      ) {
        return true
      } else {
  ...
  const total = useMemo(() => {
    if (marketList[currentIndex]?.type === 1 && amountBuy && marketList[currentIndex]?.price) {
      return new Decimal(amountBuy)
        .mul(marketList[currentIndex]?.price)
  ...
  const handleDelist = async () => {
    contractMarket.cancelListed(marketList[currentIndex].id).then((result) => {
      reloadList()
      setIsOpenBuy(false)
    })
  }
  ...
  const handleAcceptBid = async (bidId) => {
    contractMarket.acceptBid(bidId).then((result) => {
      reloadList()
      setIsOpenBuy(false)
    })
  }
  const handleCancelBid = async (bidId) => {
    contractMarket.cancelBid(bidId).then((result) => {
      reloadList()
      setIsOpenBuy(false)
    })
  }
  ...
  return (
    <>
      {isOpenBuy && (
        <div
          key={marketList[currentIndex]._id}
          className='market-popupX'
          onClick={() => setIsOpenBuy(false)}
        >
          <form className='containerX' onSubmit={handleBuy} onClick={(e) => e.stopPropagation()}>
            {/*new element: popup name */}
            {marketList[currentIndex]?.type === 1 && <h1>Checkout</h1>}
            {marketList[currentIndex]?.type === 2 && <h1>Bidding</h1>}
            ...
            <div className='contents_box'>
              <div className='form-control'>
                <img
                  className='preview-image 25mb'
                  src={marketList[currentIndex]?.asset?.metadata?.image}
                  alt=''
                />
                ...
                <div className='items_information'>
                  ...
                  {/* e:new element: Type */}
                  <p>
                    Avaiable {marketList[currentIndex]?.quantity}
                    <br />
                    {/* e:new element: Price */}
                    <strong>
                      {paymentList.map((token) => {
                        if (token.address === marketList[currentIndex]?.payment_token) {
                          return (
                            <div className='price'>
                              {new Decimal(marketList[currentIndex].price).div(
                    ...
                    <div className='input_boundingbox'>
                      ...
                      <span
                        ...
                        onClick={() => {
                          if (
                            price <=
                            new Decimal(marketList[currentIndex].price)
                              .div(new Decimal(10).pow(18))
                              .toNumber()
                          )
                            return
              ...
              {isOwner && (
                <button
                  className='buttonX'
                  onClick={() => handleDelist(marketList[currentIndex].id)}
                >
                  Delisting
                </button>
              )}
      ...
      <div className='main-wrapper'>
        <div className='box-slide-artwork'>
          ...
          <div
            className='box-img'
            onClick={() => window.open(marketList[currentIndex]?.asset?.metadata?.image, '_blank')}
          >
            <img src={zoom} alt='' />
          </div>
        </div>
        <div className='box-artist-wrapper'>
          <div className='artist-content'>
            <div className='artist-content-top'>
              <p className='text-artist'>NFT details</p>
              <h2 className='title-artist'>{marketList[currentIndex]?.asset?.metadata?.name}</h2>
              <p className='desc-artist'>
                <span className='desc'>
                  {new Decimal(
                    marketList[currentIndex]?.price ? marketList[currentIndex]?.price : 0
                  )
                    .div(new Decimal(10).pow(18))
                    .toString()}{' '}
                  KDG
                ...
                <span className='txt'>{marketList[currentIndex]?.quantity}</span>
              </p>
            </div>
              ...
              <div className='desciption-artist'>
                <p className='desc'>
                  ...
                  <span className='color-fff'>
                    {' '}
                    {marketList[currentIndex]?.owner?.kyc?.last_name
                      ? marketList[currentIndex]?.owner?.kyc?.last_name +
                        ' ' +
                        marketList[currentIndex]?.owner?.kyc?.first_name
                      : marketList[currentIndex]?.owner?.address}{' '}
                  </span>
                </p>
                {/* <p className="desc">
                ...
                <p className='desc'>
                  ...
                  <span className='color-fff'>
                    {' '}
                    {new Date(marketList[currentIndex]?.asset?.time).toDateString()}
                  </span>
                </p>
                <p className='desc mar-t-10'>
                  ...
                  <span className='color-fff d-block mar-t-10'>Channel: KingLive</span>
                </p>
              </div>
            </div>
            <div className='artist-content-bottom'>
              <div className='artist-contract'>
                <div className='contract-item'>
                  <p className='text-contract'>NFT Contract ID:</p>
                  <p className='address-contract'>
                    {marketList[currentIndex]?.asset?.collection_id?.substring(0, 5) +
                      '...' +
                      marketList[currentIndex]?.asset?.collection_id?.substring(
                        marketList[currentIndex]?.asset?.collection_id.length - 9,
                        marketList[currentIndex]?.asset?.collection_id.length
                      )}{' '}
                    <img src={address} alt='' />
                  </p>
                </div>
                <div className='contract-item'>
                  <p className='text-contract'>Token ID:</p>
                  <p className='address-contract'>
                    {marketList[currentIndex]?.asset?.id} <img src={address} alt='' />
                  </p>
                </div>
                <div className='contract-item'>
                  <p className='text-contract'>Creator's Address:</p>
                  <p className='address-contract'>
                    {marketList[currentIndex]?.asset?.owner.address.substring(0, 5) +
                      '...' +
                      marketList[currentIndex]?.asset?.owner?.address.substring(
                        marketList[currentIndex]?.asset?.owner?.address.length - 9,
                        marketList[currentIndex]?.asset?.owner?.address.length
                      )}{' '}
                    <img src={address} alt='' />
                  </p>
                </div>
        ...
        <div className='box-bidding'>
          <div className='content-bidding'>
            <h2 className='title-bidding'>Bidding</h2>
                ...
                {marketList[currentIndex]?.bid_orders.map((bid) => {
                  if (bid.status !== 2)
                    return (
                      <tr>
                        <td>
                          {bid.from?.kyc?.last_name
                            ? bid.from?.kyc?.last_name + ' ' + bid.from?.kyc?.first_name
                            : bid.from?.address?.substring(
                                bid.from?.address?.length - 8,
                                bid.from?.address?.length
                              )}
                        </td>
                        <td>{bid.quantity}</td>
                        <td>
                        ...
                        <td>
                          ...
                          {isOwner && bid?.status === 1 && <div>Accepted</div>}
        ...
        <div className='box-trading'>
          <div className='content-trading'>
            <h2 className='title-trading'>Trading History</h2>
                        ...
                        <div className='info-user'>
                          <div className='avt'>
                            <img
                              alt=''
                              src={
                                buy.from?.kyc?.avatar?.path
                                  ? `${STORAGE_DOMAIN}${buy.from?.kyc?.avatar?.path}`
                                  : avatarDefault
                              }
                            />
                          </div>
                          <div className='info'>
                            <span className='info-name'>
                              {buy.from?.kyc?.last_name
                                ? buy.from?.kyc?.last_name + ' ' + buy.from?.kyc?.first_name
                                : '0x.....' +
                                  buy.from?.address?.substring(
                                    buy.from?.address?.length - 8,
                                    buy.from?.address?.length
                                  )}
                            </span>
                            <span className='info-date'>{new Date(buy?.time).toDateString}</span>
                          </div>
                        </div>
                      </td>
                      <td>{buy?.quantity}</td>
